package org.cloudbus.cloudsim.examples.dvfs;

/*
 * Title: CloudSim Toolkit Description: CloudSim (Cloud Simulation) Toolkit for Modeling and Simulation of Clouds Licence: GPL -
 * http://www.gnu.org/copyleft/gpl.html Copyright (c) 2009, The University of Melbourne, Australia
 */

/*
 * @author Guérout TomExperiment to compare Dvfs behaviour and power consumption resultat between CloudSim and a Real Host.
 */
...
/**
 * An example of a heterogeneous DVFS-enabled data center: the voltage and clock frequency of the CPU are adjusted ideally according to current resource
 * requirements.
 */
public class CloudSimExampleDVFS_complex {
     ...
     * @param userId
     * @param nb_cloudlet
     *            : number of cloudlet we want to create
     * @param IdShift
     *            : id of cloudlet
     * @param type_cl
     *            : type of cloudlet related to the XML file
    ...
    private static List<Vm> createVms(int userId, int nb_vm, int IdShift, int type_vm) {
        ...
        for (int i = no_cur_vm; i < (offset + nb_vm); i++) {
            vms.add(new Vm((IdShift + i), userId, mips[type_vm], pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared()));
            no_cur_vm++;
            System.out.println("VM created  // No VM =  " + no_cur_vm + "  //  List Vm size = " + vms.size() + "  // ID Shift+1 = " + (IdShift + i) + "id vm = " + vms.get(0)
                .getId());
        }
    ...
    private static PowerDatacenter createDatacenter(String name) throws Exception {
        ...
        ArrayList<Double> freqs = new ArrayList<Double>(); // frequencies available by the CPU
        ...
        for (int i = 0; i < hostsNumber; i++) {
            ...
            hostList.add(new PowerHost(i, new RamProvisionerSimple(ram), new BwProvisionerSimple(bw), storage, peList, new VmSchedulerTimeShared(peList), new PowerModelSpecPower_BAZAR(peList), false, enableDVFS
        ...
        try {
            powerDatacenter = new PowerDatacenter(name, characteristics, new PowerVmAllocationPolicySimpleWattPerMipsMetric(hostList), new LinkedList<Storage>(), 0.01); // fix
                                                                                                                                                                         // to
                                                                                                                                                                         // 0.1
                                                                                                                                                                         // as
                                                                                                                                                                         // the
    ...
    public static class GlobalBroker extends SimEntity {
        ...
        /*
         * This GlobalBroker class is used to create successively broker that launch one or multiple VM. Indeed, a broker can "contain" one or more VM. One (and
         * only one) cloudlet is attached to one VM. The reasone is if we use only one VM, this has to have to the same size as the Host to be able to generate
         * 100% cpu load. In our simulation we want to generate a pre-defined scenario, with multiple predefine phase. It's more convinient to create different
         * type of VM and different type of Cloudlet. By mixing these different type of VM/Cloudlet we can easaly manage the CPU load of the simulation. The
         * size of the VM define the grain with we are able to increase/decrease the CPU load, and the Cloudlet length determine the duration of its execution,
         * depend on which type of VM it is launched. DVFS ---- This scenario is defined at Maximum frequency. So all broker is set to have the good CPU load
         * curve at Maximum frequency. When the CPU is decrease, the execution of VM/CloudLet could be slowed and the sequence of brokers must take into account
         * this slowdown. If we create broker with STATIC delay start, it could be works at Maximum frequency, but not at all for another (lower) frequency. So,
         * the launching of a broker could be create dynamically at the end of another broker. (Event Post Broker)
         */
