...
public class SchedulingHelper {
	...
	public static double simulation(Chromosome chromosome) {
		SchedulingChrom chrom = (SchedulingChrom) chromosome;
		double fitness = -1;
		List<Vm> vms;
		...
		try {
			...
			DatacenterBroker broker = (RealtimeDatacenterBroker) RealtimeHelper.createBroker();
			int brokerId = broker.getId();

			vms = RealtimeHelper.createVmList(brokerId, chrom.getChromosomeDim());
			List<Cloudlet> cloudletList = RealtimeHelper.createRealtimeCloudlet(brokerId, vms, chrom.getChromosomeDim());
			hosts = RealtimeHelper.createHostList(RealtimeConstants.NUMBER_OF_HOSTS);

			PowerDatacenter datacenter = (PowerDatacenter) SchedulingHelper.createDatacenter("Datacenter", PowerDatacenter.class, hosts, "ga_init", chrom);
			...
			fitness = instructions/(energy*3600*1000);
			chrom.setTdr(tdr);
			chrom.setDmr(dmr);
			chrom.setEnergy(energy);
