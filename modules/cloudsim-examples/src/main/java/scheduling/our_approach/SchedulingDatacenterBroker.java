...
public class SchedulingDatacenterBroker extends PowerDatacenterBroker {
	int vmIndex;
	ArrayList<Vm> declinedVms = new ArrayList<Vm>();

	public SchedulingDatacenterBroker(String name) throws Exception {
		super(name);
	}
	...
	protected void createVmsInDatacenter(int datacenterId) {
		...
		for (Vm vm : getVmList()) {
			if (!getVmsToDatacentersMap().containsKey(vm.getId())) {
				...
				send(datacenterId, ((SchedulingVm)vm).getCloudlet().getStartTime(), CloudSimTags.VM_CREATE_ACK, vm);
				requestedVms++;
			}
		}
	...
	protected void processVmCreate(SimEvent ev) {
		...
		if (result == CloudSimTags.TRUE) {
			...
			SchedulingVm rvm = (SchedulingVm) PowerVmList.getById(getVmsCreatedList(), vmId);
	...
	protected void submitCloudlet(SchedulingCloudlet cloudlet) {
		if (cloudlet.getStartTime() <= CloudSim.clock()) {
			Vm vm = null;
			if (cloudlet.getVmId() == -1) {
				vm = getVmsCreatedList().get(vmIndex);
				((SchedulingVm)vm).setCloudlet(cloudlet);
	...
	private void submitCloudlet(SimEvent ev) {
		SchedulingCloudlet rc = (SchedulingCloudlet) ev.getData();
		Vm vm = null;
	...
	protected void submitCloudlets() {
		...
		for (Cloudlet cloudlet : getCloudletList()) {
			SchedulingCloudlet rc = (SchedulingCloudlet) cloudlet;
