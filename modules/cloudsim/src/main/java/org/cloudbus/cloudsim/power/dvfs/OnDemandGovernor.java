 ...
 * OnDemand Governor.
 * 
 * This class override "specificDecision" method to return the right value in
 * relation with OnDemand mod behavior, especially for the SamplingDownFactor
 * parameter
 * 
...
public class OnDemandGovernor extends AbstractGovernor {
	/*
	 * Value between 0 and 100 wait "samplingDownFactor" iteration before
	 * decrease frequency
	 */
	private int samplingDownFactor = 100;

	/*
	 * Count iteration before decrease frequency Until downIt <
	 * samplingDownFactor
	 */
	...
	public int SpecificDecision(double utilPe) {
		Log.printLine("spec descision ondemand , util = " + utilPe);
		int desc = decision(utilPe);
		Log.printLine("desc == " + desc);
		if (desc == -1) {
			if (IsTimetoDown())
				setDownItReset();
			else {
	...
	private void ConfigParameters(HashMap<String, Integer> configOnDemand) {
		if (configOnDemand.containsKey("up_threshold")) {
			...
			Log.printLine("OnDemand UP_Threshold new value : " + Threshold);
		}
		if (configOnDemand.containsKey("sampling_down_factor")) {
			...
			Log.printLine("OnDemand SamplingDownFactor new value : " + samplingDownFactor);
		}
	}
