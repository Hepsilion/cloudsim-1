...
/**
 * This class defines a "watt per mips" metric too choose a Host when all are
 * overused.
 * 
 * The aim is to choose the Host that have the best ratio Watt/Mips weighted by
 * the theoretical capacity to host a new VM
 * 
 ...
 */
public class PowerVmAllocationPolicySimpleWattPerMipsMetric extends
	...
	private double MetricWattPerMips(PowerHost h) {
		...
		for (Pe pe : h.getPeList()) {
			...
			// tmp_metric+= (Pe_Pmax - Pe_Pmin) * pe_p.getUtilization() /
			// pe_p.getAvailableMips();
			tmp_metric += (Pe_Pmax - Pe_Pmin) * SumVmMaxMips / MaxMipsHost;

			// Log.printLine("tmpMetricPe = (" + Pe_Pmax + " - " + Pe_Pmin +
			// " ) * " + SumVmMaxMips + " / " + MaxMipsHost);
