 ...
 * Governor DVFS Class
 * 
 * The Governor decides if it's time to increase/decrease the CPU frequency,
...
public abstract class AbstractGovernor {
	...
	/*
	 * This methode is overwited on OnDemand and Conservative governor because
	 * they have special behaviour A decision of increase or decrease frequency
	 * has not the same effect on the frequency (different step) with OnDemand
	 * or with Conservative mode. Moreover, these two mode may have special
	 * Attribute to take in account.
	 */
	public int SpecificDecision(double util) {
		return 0;
	}

	/**
	 * Return the dvfs governor Decision regarding the current CPU Utilization
	 * This methode is the same for all mode, it just decide to increase or
	 * decrease frequency in relantion with Thresholds.
	 * 
